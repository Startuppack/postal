  ...
  ConfigSchema = Konfig::Schema.draw do
    group :postal do
      ...
      string :smtp_relays do
        array
        description "An array of SMTP relays in the format of smtp://user:password@host:port"
        transform do |value|
          uri = URI.parse(value)
    ...
    group :api do
      ...
      string :required_azp do
        description "If set, only JWTs whose 'azp' (authorized party) claim matches this value are accepted. Use the Keycloak client_id of your service account."
      end
    end
  end
