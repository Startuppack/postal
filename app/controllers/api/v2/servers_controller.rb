...
module Api
  module V2
    class ServersController < BaseController
      ...
      before_action(only: [:create, :update, :destroy, :suspend, :unsuspend]) do
        require_org_write!(@organization)
      end

      def index
        render json: paginate(@organization.servers.present.order(:name)).map { |s| serialize(s) }
      end
